#ifndef __FAST_FIXED__
#define __FAST_FIXED__
...
struct FastFixed {
  ...
  constexpr FastFixed(int v) : v(static_cast<StorageType>(v) << K) {}
  ...
  friend FastFixed operator*(int a, FastFixed b) { return b * a; }
...
FastFixed<N, K> operator*(FastFixed<N, K> a, FastFixed<N, K> b) {
  ...
  if constexpr (N <= 32) {
    return FastFixed<N, K>::from_raw((static_cast<int_fast64_t>(a.v) * b.v) >>
                                     K);
  } else {
    ST high = (a.v >> K) * b.v;
...
FastFixed<N, K> operator/(FastFixed<N, K> a, FastFixed<N, K> b) {
  using ST = typename FastFixed<N, K>::StorageType;
  if constexpr (N <= 32) {
    return FastFixed<N, K>::from_raw((static_cast<int_fast64_t>(a.v) << K) /
                                     b.v);
  } else {
...
FastFixed<N, K> operator-(FastFixed<N, K> x) {
  return FastFixed<N, K>::from_raw(-x.v);
}
